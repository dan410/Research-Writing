...
\todo[inline]{Need to introduce the theoretical frame work and discuss in more detail the difference between the 'start from kernel' and 'start from penalty' approach. Also, it might be good to introduce the typical function spaces here (e.g. $W_2$ and $W_2^2$)}
...
\todo[inline]{This would make more sense if you already introduced the function space $W_2$ and discussed the typical penalty and corresponding unpenalized sunspace. }
\begin{equation}
...
\]
then the vectors $\mathbf{b}^{(i)}$ contain all pairwise products of observations on the $i$th curve. We propose the following estimator \todo[inline]{I think you need to scale first term by 1/nm(m-1)}
\[
